...
class PermissionMixin:
    ...
    def has_write_permission(user):
        roler = NetBoxUserRoleWrapper(user=user)
        if roler.has_ipam_admin_writable():
            return True

        if roler.has_link_write_permission():
            return True
    ...
    def has_read_permission(user):
        roler = NetBoxUserRoleWrapper(user=user)
        if roler.has_ipam_admin_readable():
            return True

        if roler.has_link_read_permission():
            return True
