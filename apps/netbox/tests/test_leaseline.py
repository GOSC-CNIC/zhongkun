...
class LeaseLineTests(MyAPITransactionTestCase):
    ...
    def test_creat(self):
        base_url = reverse('netbox-api:link-leaseline-list')
        data = json.dumps({
    ...
    def test_update(self):
        base_url = reverse('netbox-api:link-leaseline-update-leaseline', kwargs={'id': 'test'})
        data = json.dumps({
            'private_line_number': '510GXN12603174',
            'lease_line_code': '0F0001NP',
            'line_username': '广州联通互联互通',
        ...
        base_url = reverse('netbox-api:link-leaseline-update-leaseline', kwargs={'id': id})
        response = self.client.post(base_url, data=data, content_type='application/json')
        self.assertEqual(response.status_code, 200)
