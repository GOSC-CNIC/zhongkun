...
from apps.app_net_link.managers.link import DistriFrameManager
from apps.app_net_link.models import DistributionFrame
...
class DistriFrameTests(MyAPITransactionTestCase):
    ...
    def test_list_distriframe(self):
        ...
        # user role 
        base_url = reverse('net_link-api:link-distributionframe-list')
    ...
    def test_retrieve_distriframe(self):
        ...
        # user role
        base_url = reverse('net_link-api:link-distributionframe-detail', kwargs={'id': distriframe.id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 401)
        self.client.force_login(self.user1)
        response = self.client.get(base_url)
        ...
        # data not exist
        base_url = reverse('net_link-api:link-distributionframe-detail', kwargs={'id': 'asd'})
        response = self.client.get(base_url)
