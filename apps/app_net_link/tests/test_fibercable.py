...
class FiberCableTests(MyAPITransactionTestCase):
    ...
    def test_creat(self):
        base_url = reverse('net_link-api:link-fibercable-list')
        data = {
            'number': 'SM-test',
            'fiber_count': '30',
            'length': '30.5',
    ...
    def test_retrieve_fibercable(self):
        ...
        # user role
        base_url = reverse('net_link-api:link-fibercable-detail', kwargs={'id': fibercable.id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 401)
        self.client.force_login(self.user1)
        response = self.client.get(base_url)
        ...
        # element not exist
        base_url = reverse('net_link-api:link-fibercable-detail', kwargs={'id': 'asd'})
        response = self.client.get(base_url)
        self.assertErrorResponse(
