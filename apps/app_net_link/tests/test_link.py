...
class LinkTests(MyAPITransactionTestCase):
    ...
    def test_list_link(self):
        ...
        # user role 
        base_url = reverse('net_link-api:link-link-list')
    ...
    def test_retrieve_link(self):
        ...
        # user role
        base_url = reverse('net_link-api:link-link-detail', kwargs={'id': link.id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 401)
        self.client.force_login(self.user1)
        response = self.client.get(base_url)
        ...
        # element not exist
        base_url = reverse('net_link-api:link-link-detail', kwargs={'id': 'asd'})
        response = self.client.get(base_url)
