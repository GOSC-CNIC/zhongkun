...
class IPv4AddressTests(MyAPITransactionTestCase):
    ...
    def test_remark(self):
        ...
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': -1})
        response = self.client.post(base_url)
        self.assertErrorResponse(status_code=400, code='InvalidArgument', response=response)
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': 2**32})
        response = self.client.post(base_url)
        self.assertErrorResponse(status_code=400, code='InvalidArgument', response=response)

        # remark
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': int(ipaddress.IPv4Address('10.8.8.6'))})
        response = self.client.post(base_url)
        ...
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': int(ipaddress.IPv4Address('127.0.0.1'))})
        query = parse.urlencode(query={'remark': 'test'})
        response = self.client.post(f'{base_url}?{query}')
        self.assertErrorResponse(status_code=403, code='AccessDenied', response=response)
        ...
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': int(ipaddress.IPv4Address('159.0.1.1'))})
        query = parse.urlencode(query={'remark': 'test'})
        response = self.client.post(f'{base_url}?{query}')
        self.assertErrorResponse(status_code=403, code='AccessDenied', response=response)

        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': int(ipaddress.IPv4Address('66.0.0.1'))})
        query = parse.urlencode(query={'remark': 'test'})
        response = self.client.post(f'{base_url}?{query}')
        self.assertErrorResponse(status_code=403, code='AccessDenied', response=response)
        ...
        base_url = reverse('net_ipam-api:ipam-ipv4address-remark', kwargs={'ipv4': int(ipaddress.IPv4Address('16.6.6.8'))})
        query = parse.urlencode(query={'remark': 'test88', 'admin_remark': 'admin remark88', 'as-admin': ''})
        response = self.client.post(f'{base_url}?{query}')
        self.assertEqual(response.status_code, 200)
        self.assertKeysIn(['id', 'ip_address', 'remark', 'admin_remark'], response.data)
    ...
    def test_list(self):
        ...
        base_url = reverse('net_ipam-api:ipam-ipv4address-list')
