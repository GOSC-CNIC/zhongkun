...
from apps.app_net_ipam.managers.ipv6_mgrs import IPv6RangeManager, IPv6RangeRecordManager
from apps.app_net_ipam.models import (
    ASN, OrgVirtualObject, IPv4Range, IPv4RangeRecord, IPRangeItem,
    IPv6RangeStrItem
)
...
class IPv6RangeRecordTests(MyAPITransactionTestCase):
    def setUp(self):
        self.user1 = get_or_create_user(username='tom@qq.com')
        self.user2 = get_or_create_user(username='lisi@cnic.cn')
    ...
    def test_list(self):
        ...
        ip_range1 = IPv6RangeManager.create_ipv6_range(
            name='已分配1', start_ip='2001:cc0::', end_ip='2001:cc0:ffff:ffff:ffff:ffff:ffff:ffff', prefixlen=32,
            asn=asn66, create_time=nt, update_time=nt, status_code=IPv4Range.Status.ASSIGNED.value,
            org_virt_obj=virt_obj1, assigned_time=nt, admin_remark='admin1', remark='remark1'
        )
        nt = dj_timezone.now()
        ip_range2 = IPv6RangeManager.create_ipv6_range(
            name='预留2', start_ip='2001:cc1::', end_ip='2001:cc1:ffff:ffff:ffff:ffff:ffff:ffff', prefixlen=32,
            asn=asn88, create_time=nt, update_time=nt, status_code=IPv4Range.Status.RESERVED.value,
            org_virt_obj=virt_obj2, assigned_time=nt, admin_remark='admin remark2', remark='remark2'
        )
        ip_range3 = IPv6RangeManager.create_ipv6_range(
            name='预留3', start_ip='8fff:ffff:ffff:ffff::', end_ip='8fff:ffff:ffff:ffff:ffff:ffff:ffff:ffff',
            prefixlen=64, asn=asn88, create_time=nt, update_time=nt, status_code=IPv4Range.Status.RESERVED.value,
            org_virt_obj=virt_obj1, assigned_time=nt, admin_remark='admin remark3', remark='remark3'
        )

        record1 = IPv6RangeRecordManager.create_record(
            user=self.user1, record_type=IPv4RangeRecord.RecordType.ADD.value,
            start_address=ip_range1.start_address, end_address=ip_range1.end_address, prefixlen=ip_range1.prefixlen,
            remark='', ip_ranges=[], org_virt_obj=None
        )

        record2 = IPv6RangeRecordManager.create_record(
            user=self.user1, record_type=IPv4RangeRecord.RecordType.SPLIT.value,
            start_address=ip_range1.start_address, end_address=ip_range1.end_address, prefixlen=ip_range1.prefixlen,
            remark='', ip_ranges=[
                IPv6RangeStrItem(start='2001:cc0::', end='2001:cc0:7fff:ffff:ffff:ffff:ffff:ffff', prefix=33),
                IPv6RangeStrItem(start='2001:cc0:7fff::', end='2001:cc0:ffff:ffff:ffff:ffff:ffff:ffff', prefix=33),
            ], org_virt_obj=virt_obj1
        )
        record3 = IPv6RangeRecordManager.create_record(
            user=self.user2, record_type=IPv4RangeRecord.RecordType.MERGE.value,
            start_address=ip_range2.start_address, end_address=ip_range2.end_address, prefixlen=ip_range2.prefixlen,
            remark='', ip_ranges=[
                IPv6RangeStrItem(start='2001:cc1::', end='2001:cc1:7fff:ffff:ffff:ffff:ffff:ffff', prefix=33),
                IPv6RangeStrItem(start='2001:cc1:7fff::', end='2001:cc1:ffff:ffff:ffff:ffff:ffff:ffff', prefix=33),
            ], org_virt_obj=virt_obj2
        )
        record4 = IPv6RangeRecordManager.create_record(
            user=self.user2, record_type=IPv4RangeRecord.RecordType.RESERVE.value,
            start_address=ip_range2.start_address, end_address=ip_range2.end_address, prefixlen=ip_range2.prefixlen,
            remark='', ip_ranges=[], org_virt_obj=virt_obj2
        )
        IPv6RangeRecordManager.create_record(
            user=self.user2, record_type=IPv4RangeRecord.RecordType.RESERVE.value,
            start_address=ip_range3.start_address, end_address=ip_range3.end_address, prefixlen=ip_range3.prefixlen,
            remark='', ip_ranges=[], org_virt_obj=virt_obj3
        )

        base_url = reverse('net_ipam-api:record-ipv6range-list')
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 401)
        ...
        self.assertEqual(response.data['results'][0]['id'], record2.id)
        self.assertKeysIn([
            'id', 'record_type', 'creation_time', 'start_address', 'end_address', 'prefixlen',
            'ip_ranges', 'remark', 'user', 'org_virt_obj'
        ], response.data['results'][0])
        self.assertEqual(response.data['results'][0]['user']['id'], self.user1.id)
        self.assertEqual(response.data['results'][0]['user']['username'], self.user1.username)
        ...
        self.assertIsInstance(response.data['results'][0]['ip_ranges'], list)
        self.assertEqual(len(response.data['results'][0]['ip_ranges']), 2)
        self.assertKeysIn(['start', 'end', 'prefix'], response.data['results'][0]['ip_ranges'][0])
        ...
        # ipv6
        query = parse.urlencode(query={'ipv6': '2001:cc0:7fff:ffff:ffff:ffff:ffff:fff0'})
        response = self.client.get(f'{base_url}?{query}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['count'], 2)
        self.assertEqual(response.data['page_num'], 1)
        ...
        query = parse.urlencode(query={'ipv6': '2001:cc1:7fff::'})
        response = self.client.get(f'{base_url}?{query}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['count'], 2)
        self.assertEqual(response.data['page_num'], 1)
