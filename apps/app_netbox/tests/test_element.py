...
from apps.app_netbox.models import OpticalFiber, ConnectorBox, DistriFramePort, LeaseLine, Element
from apps.app_netbox.managers.link_mgrs import FiberCableManager, DistriFrameManager, ConnectorBoxManager, LeaseLineManager
...
class TaskTests(MyAPITransactionTestCase):
    ...
    def test_retrieve_element(self):
        ...
        base_url = reverse('netbox-api:link-element-detail',kwargs={'id': fiber_element_id})
        ...
        # Invalid id
        base_url = reverse('netbox-api:link-element-detail',kwargs={'id': '  '})
        response = self.client.get(base_url)
        self.assertErrorResponse(status_code=400, code='InvalidArgument', response=response)

        # element not exist
        base_url = reverse('netbox-api:link-element-detail', kwargs={'id': '123'})
        response = self.client.get(base_url)
        ...
        # lease
        base_url = reverse('netbox-api:link-element-detail',kwargs={'id': lease_element_id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 200)
        ...
        # port
        base_url = reverse('netbox-api:link-element-detail',kwargs={'id': port_element_id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 200)
        ...
        # box
        base_url = reverse('netbox-api:link-element-detail',kwargs={'id': box_element_id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 200)
