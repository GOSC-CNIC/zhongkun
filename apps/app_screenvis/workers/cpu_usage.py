...
class HostCpuUsageWorker:
    ...
    def do_tasks(self, tasks: list):
        ...
        for r in results:
            if isinstance(r, tuple) and len(r) == 3:
                ...
                obj = HostCpuUsage(timestamp=now_timestamp, unit_id=unit_id, value=usage_value)
        ...
        if objs:
            try:
                objs = HostCpuUsage.objects.bulk_create(objs=objs, batch_size=200)
    ...
    @staticmethod
    def create_cpuusage_log(timestamp: int, unit_id: str, usage_value: float):
        try:
            obj = HostCpuUsage(timestamp=timestamp, unit_id=unit_id, value=usage_value)
    ...
    def do_update_tasks(self, tasks: list):
        ...
        for r in results:
            if isinstance(r, tuple) and len(r) == 2:
                ...
                if isinstance(r_value, (int, float)):
                    ...
                    unit_err_map[obj.unit_id] = r_value
            else:
                print(r)
                continue
        ...
        if update_objs:
            try:
                ok_count = HostCpuUsage.objects.bulk_update(update_objs, fields=['value'])
            except Exception as exc:
                pass
    ...
    def service_down_unit_ids(self, unit_objs_map: dict, before_minutes: int, cycle_minutes: int, now_ts: int):
        ...
        for unit_id, objs in unit_objs_map.items():
            tss = [i.timestamp for i in objs if i.value < 0]
