...
class MetricTiDBTests(MyAPITestCase):
    ...
    def test_query(self):
        ...
        r = self.query_ok_test(unit_id=tidb_unit_id, query_tag=TiDBQueryChoices.SERVER_CPU_COUNT.value)
        self.assertTrue(len(r.data[TiDBQueryChoices.SERVER_CPU_COUNT.value]) == 1)
