...
class AlertTests(MyAPITestCase):
    ...
    def test_list(self):
        ...
        alert1 = AlertModel(
            ...
            order=None, count=1, creation=nt_ts, modification=int(nt_ts),
        ...
        rl_alert2 = ResolvedAlertModel(
            ...
            order=None, count=2, creation=nt_ts, modification=int(nt_ts),
            first_notification=None, last_notification=None
        )
        ...
        alert3 = AlertModel(
            ...
            start=int(nt_ts), end=int(nt_ts), recovery=int(nt_ts), status=AlertModel.AlertStatus.FIRING.value,
            order=None, count=3, creation=nt_ts, modification=int(nt_ts),
            first_notification=None, last_notification=None
        )
        ...
        response = self.query_response(querys={'dc_id': 'a2'})
        self.assertErrorResponse(status_code=400, code='InvalidArgument', response=response)
        ...
        # dc_id
        response = self.query_response(querys={'dc_id': 0})
        self.assertEqual(response.status_code, 200)
        self.assertKeysIn(['has_next', 'next_marker', 'marker', 'page_size', 'results'], response.data)
        self.assertFalse(response.data['has_next'])
        self.assertIsNone(response.data['next_marker'])
        ...
        response = self.query_response(querys={'dc_id': odc1.id})
        self.assertEqual(response.status_code, 200)
        self.assertKeysIn(['has_next', 'next_marker', 'marker', 'page_size', 'results'], response.data)
        self.assertFalse(response.data['has_next'])
        self.assertIsNone(response.data['next_marker'])
        ...
        response = self.query_response(querys={'dc_id': odc2.id})
        self.assertEqual(response.status_code, 200)
        self.assertKeysIn(['has_next', 'next_marker', 'marker', 'page_size', 'results'], response.data)
        self.assertFalse(response.data['has_next'])
        self.assertIsNone(response.data['next_marker'])
        ...
        # status
        response = self.query_response(querys={'status': 'firing', 'dc_id': odc2.id})
        self.assertEqual(response.status_code, 200)
        self.assertKeysIn(['has_next', 'next_marker', 'marker', 'page_size', 'results'], response.data)
        self.assertFalse(response.data['has_next'])
        self.assertIsNone(response.data['next_marker'])
