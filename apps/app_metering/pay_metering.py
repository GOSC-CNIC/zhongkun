...
class PayMeteringDisk:
    def __init__(self, app_id: str, pay_date: date = None):
        """
        :param pay_date: 指定只扣费那个计费日的云硬盘日结算单
        """
        self.app_id = app_id
        self.pay_date = pay_date
        self.count = 0
    ...
    def start_print(self):
        print('开始扣费云硬盘日结算单')
        if self.pay_date:
            print(f'指定只扣费{self.pay_date}日期的云硬盘日结算单')

        print(f'本次要扣费的云硬盘日结算单总数：{self.count}')

    def run(self):
        queryset = self.get_disk_statement_queryset()
        self.count = queryset.count()
        self.start_print()

        last_creation_time = None
        while True:
            meterings = self.get_disk_statements(creation_time_gt=last_creation_time)
            m_length = len(meterings)
            if m_length == 0:
                break
            ...
            print(f'扣费 {m_length} 云硬盘日结算单.')

        print(f'总数：{self.count}, 扣费成功：{self.success_count}, 扣费失败：{self.failed_count}.')

    def get_disk_statements(self, creation_time_gt=None, limit: int = 100):
        queryset = self.get_disk_statement_queryset()
    ...
    def get_disk_statement_queryset(self):
        queryset = DailyStatementDisk.objects.select_related(
            'service').filter(payment_status=PaymentStatus.UNPAID.value)
        if self.pay_date:
            queryset = queryset.filter(date=self.pay_date)
    ...
    def do_pay_one_bill(self, bill: DailyStatementDisk):
        pay_mgr = MeteringPaymentManager()
        remark = f'disk, {bill.date}'
        try:
            pay_mgr.pay_daily_statement_bill(
                daily_statement=bill, app_id=self.app_id, subject='云硬盘按量计费',
                executor='metering', remark=remark, required_enough_balance=False
            )
        except Exception as exc:
            try:
                ...
                print(f'[Failed] 云硬盘日结算单（id={bill.id}）扣费失败，{exc}')
                return False

        return True


class PayMeteringWebsite:
    def __init__(self, app_id: str, pay_date: date = None):
        """
        :param pay_date: 指定只扣费那个计费日的站点监控日结算单
        """
        self.app_id = app_id
        self.pay_date = pay_date
        self.count = 0
    ...
    def start_print(self):
        print('开始扣费站点监控日结算单')
        if self.pay_date:
            print(f'指定只扣费{self.pay_date}日期的站点监控日结算单')

        print(f'本次要扣费的站点监控日结算单总数：{self.count}')

    def run(self):
        queryset = self.get_site_statement_queryset()
        self.count = queryset.count()
        self.start_print()

        last_creation_time = None
        while True:
            meterings = self.get_site_statements(creation_time_gt=last_creation_time)
            m_length = len(meterings)
            if m_length == 0:
                break
            ...
            print(f'扣费 {m_length} 站点监控日结算单.')

        print(f'总数：{self.count}, 扣费成功：{self.success_count}, 扣费失败：{self.failed_count}.')

    def get_site_statements(self, creation_time_gt=None, limit: int = 100):
        queryset = self.get_site_statement_queryset()
    ...
    def get_site_statement_queryset(self):
        queryset = DailyStatementMonitorWebsite.objects.filter(payment_status=PaymentStatus.UNPAID.value)
        if self.pay_date:
            queryset = queryset.filter(date=self.pay_date)

        return queryset

    def do_pay_one_bill(self, bill: DailyStatementMonitorWebsite):
        pay_mgr = MeteringPaymentManager()
        remark = f'website monitor, {bill.date}'
        try:
            pay_mgr.pay_daily_statement_bill(
                daily_statement=bill, app_id=self.app_id, subject='站点监控按量计费',
                executor='metering', remark=remark, required_enough_balance=False
            )
        except Exception as exc:
            try:
                ...
                print(f'[Failed] 站点监控日结算单（id={bill.id}）扣费失败，{exc}')
                return False
