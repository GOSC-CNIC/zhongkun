...
class TaskTests(MyAPITransactionTestCase):
    ...
    def test_list_task(self):
        ...
        task2 = TaskManager.create_task(
            ...
            task_status=Task.TaskStatus.DELETED
        )
        ...
        # query "task_status"
        query = parse.urlencode(query={'task_status': 'abc'})
        response = self.client.get(f'{base_url}?{query}')
        self.assertEqual(response.status_code, 400)
        query1 = parse.urlencode(query={'task_status': Task.TaskStatus.DELETED})
        query2 = parse.urlencode(query={'task_status': Task.TaskStatus.NORMAL})
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['task_status'], Task.TaskStatus.DELETED)
    ...
    def test_retrieve_task(self):
        ...
        # user role
        base_url = reverse('api:link-task-detail', kwargs={'id': task1.id})
        response = self.client.get(base_url)
        self.assertEqual(response.status_code, 401)
        self.client.force_login(self.user1)
        response = self.client.get(base_url)
        ...
        # Invalid id
        base_url = reverse('api:link-task-detail', kwargs={'id': '  '})
        response = self.client.get(base_url)
        self.assertErrorResponse(status_code=400, code='InvalidArgument', response=response)

        # element not exist
        base_url = reverse('api:link-task-detail', kwargs={'id': 'asd'})
        response = self.client.get(base_url)
        self.assertErrorResponse(status_code=404, code='TaskNotExist', response=response)
